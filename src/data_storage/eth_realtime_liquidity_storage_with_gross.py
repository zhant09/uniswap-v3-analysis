    ...
    while True:
        ...
        try:
            ...
            for item in tick_data:
                tick_mapping[int(item["tickIdx"])] = [int(item["liquidityNet"]), int(item["liquidityGross"])]
        except Exception as ex:
            error_times += 1
            if error_times == 3:
...
def get_liquidity_data(dt: datetime, pool_info: PoolInfo, tick_mapping: dict):
    ...
    while tick <= max_tick:
        ...
        liquidity_pair = tick_mapping.get(tick, [0, 0])
        liquidity_delta = liquidity_pair[0]
        liquidity_gross = liquidity_pair[1]
        ...
        line_dict["liquidity_gross"] = liquidity_gross
...
def insert_into_db(insert_data):
    sql = """INSERT INTO `pool_liquidity` (`pool_id`, `chain`, `datetime`, `fee_tier`, `tick_spacing`, `token0`, 
        `token1`, `decimals0`, `decimals1`, `bottom_tick`, `top_tick`, `liquidity_net`, `liquidity_gross`, `liquidity`,
         `bottom_price`, `top_price`, `locked_amount0`, `locked_amount1`, `is_current_tick`) VALUES (%(pool_id)s, 
         %(chain)s, %(datetime)s, %(fee_tier)s, %(tick_spacing)s, %(token0)s, %(token1)s, %(decimals0)s, %(decimals1)s, 
        %(bottom_tick)s, %(top_tick)s, %(liquidity_net)s, %(liquidity_gross)s, %(liquidity)s, %(bottom_price)s, 
        %(top_price)s, %(locked_amount0)s, %(locked_amount1)s, %(is_current_tick)s) """
